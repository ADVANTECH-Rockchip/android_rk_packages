...
void *qualcomm_serial_log_thread(void *arg)
{
    ...
    while(g_is_qxdm_logging)
    {
        ...
        if(!h_logfile)
        {
            ...
            {
                unsigned long long available_space = disk_info.f_bavail/1024*disk_info.f_bsize/1024;		//uint M 
                printf("%s availabe space = %d M\n", g_str_qualcomm_log_dir, available_space);
                if(available_space < 3)				//small than 3, tips no space for Qlog
                {
